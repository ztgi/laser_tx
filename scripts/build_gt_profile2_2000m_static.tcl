# Build an isolated static 2000 Mb/s GT TX Profile 2 bitstream.
#
...
set out_root [file join $project_dir reports gt_profile2_2000m_static]
set copied_project_dir [file join $project_dir _p2_2g_vivado]
set copied_project_name p2
...
set profile2_top [file join $project_dir rtl laser_tx_board_top_profile2_2000m.v]
set profile2_gt [file join $project_dir laser_tx.srcs sources_1 new laser_gt_tx_profile2_2000m.v]
set profile2_usrclk [file join $project_dir laser_tx.srcs sources_1 new laser_gt_usrclk_profile2_2000m.v]
set profile2_xdc [file join $project_dir constraints laser_tx_gt_profile2_2000m.xdc]
set profile0_xdc [file join $project_dir constraints laser_tx_gt_profile0.xdc]
set impl_pre_tcl [file join $script_dir gt_profile2_2000m_impl_pre.tcl]
...
set compare_script [file join $script_dir create_gtwizard_2000m_compare.tcl]
set profile2_xci [file join $project_dir reports gt_dynamic_rate_2g_static gtwizard_2000m_compare copied_ip gtwizard_0.xci]
set profile2_prop_report [file join $project_dir reports gt_dynamic_rate_2g_static gtwizard_2000m_compare gtwizard_2000m_selected_properties.txt]
...
require_file $profile2_top "Profile 2 top"
require_file $profile2_gt "Profile 2 GT wrapper"
require_file $profile2_usrclk "Profile 2 user clocking"
require_file $profile2_xdc "Profile 2 XDC"
require_file $impl_pre_tcl "Profile 2 implementation pre-hook"
require_file $compare_script "2000M comparison generator"
...
# Ensure the isolated 2000M comparison XCI exists and is current.
source $compare_script
require_file $profile2_xci "Isolated 2000M comparison XCI"
require_file_regex $profile2_prop_report {gt0_val_tx_line_rate = 2\.0} "2000M selected property report"
require_file_regex $profile2_prop_report {gt0_val_cpll_txout_div = 2} "2000M selected property report"
...
# makes the copied project explicitly static-2000M while the source Profile 0
# XCI remains untouched.
require_file $copied_project_xci "Copied project local GT Wizard XCI"
file copy -force $profile2_xci $copied_project_xci
...
add_files -fileset constrs_1 -norecurse $profile2_xdc

add_files -fileset sources_1 -norecurse [list $profile2_top $profile2_gt $profile2_usrclk]
set_property top laser_tx_board_top_profile2_2000m [get_filesets sources_1]

# Create a dedicated static-2000M GT debug ILA in the copied project.
if {[llength [get_ips -quiet ila_gt_profile2_2000m]] == 0} {
    create_ip -name ila -vendor xilinx.com -library ip -version 6.2 -module_name ila_gt_profile2_2000m
}
set ila_ip [get_ips ila_gt_profile2_2000m]
...
set_property top laser_tx_board_top_profile2_2000m [get_filesets sources_1]
set_property top_file $profile2_top [get_filesets sources_1]
update_compile_order -fileset sources_1

if {[get_property top [get_filesets sources_1]] ne "laser_tx_board_top_profile2_2000m"} {
    build_fail "sources_1 top must be laser_tx_board_top_profile2_2000m."
}

foreach run_name {gtwizard_0_synth_1 ila_gt_profile2_2000m_synth_1} {
...
open_run impl_1
write_debug_probes -force [file join $report_dir laser_tx_board_top_profile2_2000m.ltx]
report_debug_core -full_path -file [file join $report_dir debug_cores_full_path.rpt]

set debug_summary [file join $report_dir debug_cores_summary.rpt]
set debug_fh [open $debug_summary w]
puts $debug_fh "Static Profile 2 2000M debug cores"
puts $debug_fh "Generated: [clock format [clock seconds]]"
puts $debug_fh ""
foreach core [get_debug_cores] {
    puts $debug_fh "DEBUG_CORE $core"
}
close $debug_fh
...
set bit_src [file join $impl_dir laser_tx_board_top_profile2_2000m.bit]
set ltx_src [file join $impl_dir laser_tx_board_top_profile2_2000m.ltx]
set debug_ltx_src [file join $impl_dir debug_nets.ltx]
require_file $bit_src "Profile 2 bitstream"
if {![file exists $ltx_src] && [file exists $debug_ltx_src]} {
    set ltx_src $debug_ltx_src
}
require_file $ltx_src "Profile 2 LTX"

file copy -force $bit_src [file join $artifact_dir laser_tx_board_top_profile2_2000m.bit]
file copy -force $ltx_src [file join $artifact_dir laser_tx_board_top_profile2_2000m.ltx]

# Profile 0 guard after build.
require_file_regex $profile0_gen_gt {\.TXOUT_DIV\s*\(\s*8\s*\)} "Profile 0 generated GT HDL after Profile 2 build"
require_file_regex $profile0_gen_xml {gt0_val_tx_line_rate} "Profile 0 generated GT XML after Profile 2 build"
require_file_regex $profile0_gen_xml {>0[.]5<} "Profile 0 generated GT XML after Profile 2 build"

puts "INFO: Static 2000M Profile 2 bitstream completed."
puts "INFO: BIT: [file join $artifact_dir laser_tx_board_top_profile2_2000m.bit]"
puts "INFO: LTX: [file join $artifact_dir laser_tx_board_top_profile2_2000m.ltx]"

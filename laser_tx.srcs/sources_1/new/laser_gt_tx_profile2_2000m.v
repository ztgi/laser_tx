...
// Fixed compile-time GT TX Profile 2 for static 2000 Mb/s validation.
//
// This module is deliberately parallel to laser_gt_tx_profile0, but uses the
// 2000M user-clock MMCM parameters and reports profile ID 2.  It contains no
// runtime rate switching path, no GTX DRP writer and no MMCM DRP writer.
module laser_gt_tx_profile2_2000m (
    input  wire        ctrl_clk,
    input  wire        ctrl_rst,
    ...
    localparam [2:0] GT_PROFILE_ID = 3'd2;
    ...
    laser_gt_usrclk_profile2_2000m u_tx_usrclk_profile2_2000m (
    ...
    // Dedicated static-2000M GT debug ILA.  This is sampled by TXUSRCLK2, so
    // the divided debug probe should toggle every 16 TXUSRCLK2 samples.  It is
    // intended for static Profile 2 board bring-up only.
    ila_gt_profile2_2000m u_ila_gt_profile2_2000m (
        .clk    (txusrclk2),
        .probe0 (gt_ready_tx),
